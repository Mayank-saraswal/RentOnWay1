...
const Login = () => {
  ...
  const [userRole, setUserRole] = useState('customer'); // 'customer', 'seller', or 'delivery'
  ...
  // Check if we're on a specific login page
  useEffect(() => {
  ...
  // Setup recaptcha when phone login is selected
  useEffect(() => {
    if (loginMethod === 'phone' && !showOtpVerification) {
      const recaptchaContainer = document.getElementById('recaptcha-container');
      if (recaptchaContainer && !window.recaptchaVerifier) {
        setupPhoneAuth('recaptcha-container');
      }
    }
  }, [loginMethod, setupPhoneAuth, showOtpVerification]);
  ...
  const handleFacebookLogin = async () => {
    ...
    try {
      const result = await loginWithFacebook(userRole);
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      const result = await sendOTP(formattedPhone, userRole);
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      if (otpType === 'phone') {
        result = await sendOTP(otpDestination, userRole);
      } else {
        result = await sendOTP(otpDestination, 'login');
      }
      
      if (result.success) {
        toast.success(`OTP resent to your ${otpType}`);
      } else {
        toast.error(result.message || `Failed to resend OTP to your ${otpType}`);
      }
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-md">
        ...
        <div className="mt-6 text-center text-sm">
          {userRole !== 'seller' && (
            <Link to="/seller-login" className="text-primary hover:text-primary-dark font-medium">
              Seller Login
            </Link>
          )}
          {userRole !== 'delivery' && (
            <span className={userRole !== 'seller' ? ' ml-4' : ''}>
              <Link to="/delivery-login" className="text-primary hover:text-primary-dark font-medium">
